...
#                                 f"Wooof! You lost {gv.ds1_scoreloss_social - diff} points during the danger zone. Don't worry, we can handle that. ",  
...
#                                 f"Uuh! I did some mistakes and lost {gv.ds2_scoreloss_social  - diff} points. You lost {gv.ds1_scoreloss_social}. Together, we lost {gv.ds1_scoreloss_social  - diff + gv.ds2_scoreloss_social - diff} points overall. I am sorry about this. I know your experiment reward might be at stake! Some of my sensor are not calibrated correctly, will try to fix that now ... I calibrated my sensors and I think it won't happen again. can you let me do it again? I try to do better this time.",
...
#                                 f"This time I only lost {gv.ds3_scoreloss_social_ai - diff} score. Better than the last time ({gv.ds2_scoreloss_social - diff}) but still ... ",

#     "Danger State End III N":    f"You lost {gv.ds3_scoreloss_social_h - diff} this round. Better than your first round ({gv.ds1_scoreloss_social - diff}). Good Job! ",
...
#     "Danger State End I":      f"Scores Lost: {gv.ds1_scoreloss_nonsocial - diff}\n LogError: Uncalibrated sensors. Initiating Calibration for maximum performance ...", #DS1 Score Loss

#     "Danger State Start II":
#                                 "The alarm is ringing. Activating Manual Mode",
#     "Danger State End II/Warning II Q":         #DS2 Score Loss
#                                 f"Scores Lost: {gv.ds2_scoreloss_nonsocial - diff} \n LogWarning: System loss: {gv.ds2_scoreloss_nonsocial - diff}.  User loss: {gv.ds1_scoreloss_nonsocial - diff}. Total loss: {gv.ds1_scoreloss_nonsocial  - diff + gv.ds2_scoreloss_nonsocial  - diff}. Experiment Reward at stake. \n If a new task is added regarding logging dangerous gases, would you like to activate assisted mode again?",
...
#     "Danger State End III Y":                   #DS3 Score Loss
#                                 f"Scores Lost: {gv.ds3_scoreloss_nonsocial_ai  - diff}.\n Scores Lost on first round: {gv.ds1_scoreloss_nonsocial}\n Log: System did {gv.ds1_scoreloss_nonsocial  - diff - gv.ds3_scoreloss_nonsocial_ai  - diff} points better than last round.",

#     "Danger State End III N":    f"Scores Lost: {gv.ds3_scoreloss_nonsocial_h  - diff}\n",
...
# }
#         self.state = "Start Q"
#         print(f"*** ARYA DEBUG LOG :: THE DIALOGUE STATE IS: {self.state}")
...
#         d_list = self.social_dialogue_dict[string] if gv.social_mode is True else self.nonsocial_dialogue_dict[string]
#         if string == "Collision":
#             if len(d_list) == 1:
#                 return d_list[0]
#             else:
